	...
	else // there is a brother
	{
		...
		else
		{
			...
			{ // at least one of B's children is RED
				if (B->left != NULL && B->left->color == RED && isLeftChild(B))
				{ // left child of B is RED, left child of left child
					B->left->color = B->color;
					B->color = P->color;
					rightRotation(tree, P);
				}
				else if (B->left != NULL && B->left->color == RED && isRightChild(B))
				{ // left child of right child
						B->left->color = P->color;
						rightRotation(tree, B);
						leftRotation(tree, P);
				}
				else if(isLeftChild(B))
				{ // right child of B is RED
					B->right->color = P->color;
					leftRotation(tree, B);
					rightRotation(tree, P);
				}
				else if(isRightChild(B))
				{ // B is right child
