...
def gen_files():
	for i in range(NUMBER_OF_FILES):
		...
		people = list()
		pathPeople = 'in-out-example/' + str(i + 4) + '_people(big).in'
		pathMeeting = 'in-out-example/' + str(i + 4) + '_meeting(big).in'
		# people file
		...
		with open(pathMeeting, 'w') as meetingFile:
			...
			while count < MEETINGS_NUM:
				...
				for j in range(num):
					...
					count += 1
		print("finished creating file number: "+str(i))


def run_code():
	for i in range(4, NUMBER_OF_FILES + 4):
		print("running file "+str(i))
		run_string = '"cmake-build-debug\\SpreaderDetector.exe in-out-example\\{0}_people(big).in in-out-example\\{0}_meeting(big).in"'.format(i)
		os.system(run_string)
		sol_str = "in-out-example\\{0}_sol(big)".format(i)
		save_str = "cat SpreaderDetectorAnalysis.out > " + sol_str + '.out'
		os.system(save_str)
		print("finished running file num "+str(i))
